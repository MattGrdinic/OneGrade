...
"    float3 outc=(enc<=3)?pg_XYZto709(pg_DWGtoXYZ(w)):w;                                                         \n" \
"    float dg=(enc==1)?2.2f:((enc==2)?2.4f:0.0f);                                                                \n" \
"    outc.x=pg_lgg(outc.x,gain,lift,gamma,dg); outc.y=pg_lgg(outc.y,gain,lift,gamma,dg); outc.z=pg_lgg(outc.z,gain,lift,gamma,dg); \n" \
...
"    if(rolloff>0.0f && (enc<=2 || (lutN>=2 && lutMix>0.0f))){ e.x=pg_softclip(e.x,rolloff); e.y=pg_softclip(e.y,rolloff); e.z=pg_softclip(e.z,rolloff); } \n" \
