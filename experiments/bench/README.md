...
| `--bias-at` | off | render **one** frame at exactly this slider position, as `<name>-atp0906.png`. For comparing the two sides of a step, where a sweep's fixed stops will not land where you need them |
| `--hand-lift` / `--hand-gamma` / `--hand-gain` | off | simulate a manual edit after Magic Grade: set one of the three, re-derive the conditions the grade now meets, and check bias 0 gives the edit back. Prints `PRESERVED` or `LOST` |
